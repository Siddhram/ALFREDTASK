...
const level1Cards = response.data.filter((card) => card.box === 1);
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-r  ${
        colourstate === 0 ? ' from-green-500 via-blue-500 to-purple-500' : 'bg-gray-900 text-white'
    ...
    <Link to="/value3">
      ...
      <div className="w-full max-w-xl">
        {flashcards.length > 0 ? (
            ...
            >
              <div className={`flip-card ${showAnswer[card._id] ? 'flipped' : ''}`} onClick={() => handleShowAnswer(card._id)}>
                <div className="flip-card-inner">
                  <div className="flip-card-front bg-gradient-to-br from-blue-300 to-purple-500 text-white flex items-center justify-center p-6 rounded-xl shadow-xl border border-white cursor-pointer">
                    <h3 className="text-2xl font-bold text-center drop-shadow-lg">{card.question}</h3>
                  </div>
                  <div className="flip-card-back bg-gradient-to-br from-pink-300 to-red-500 text-white flex flex-col items-center justify-center p-6 rounded-xl shadow-xl border border-white">
          ...
          <p className="text-white text-xl font-semibold">🚨 No flashcards available for Level 2.</p>
        )}
      </div>
    </div>
  );
