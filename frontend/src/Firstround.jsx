  ...
  return (
    <div
      className={`min-h-screen ${
        colourstate === 0 ? ' bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500' : 'bg-gray-900 text-white'
      } p-6 flex flex-col items-center`}
    >
      <h1 className="text-5xl font-extrabold mb-10 text-white shadow-md tracking-wide drop-shadow-lg">
        ...
        {flashcards.length > 0 ? (
          flashcards.map((card) => (
            ...
            >
              <div
                className={`flip-card ${
                  showAnswer[card._id] ? 'flipped' : ''
                }`}
                onClick={() => handleShowAnswer(card._id)}
              >
                <div className="flip-card-inner">
                  <div className="flip-card-front bg-gradient-to-br from-yellow-300 to-orange-500 text-white flex items-center justify-center p-6 rounded-xl shadow-xl border border-white cursor-pointer">
                    <h3 className="text-2xl font-bold text-center drop-shadow-lg">
                      Question : {card.question} ?
                    </h3>
                  </div>
                  <div className="flip-card-back bg-gradient-to-br from-green-300 to-teal-500 text-white flex flex-col items-center justify-center p-6 rounded-xl shadow-xl border border-white">
                    <p className="text-xl font-semibold text-center drop-shadow-lg mb-4">
                      Answer : {card.answer}
                    </p>
          ...
          <p className="text-white text-xl font-semibold">
            🚨 No flashcards available for Level 1.
          </p>
        )}
      </div>
    </div>
